...
import { useFollowCam } from "./FollowCamController";
...
const CharacterController = (
  {
    ...
    camFollowMult = 11, // Ajuste para seguimiento de cámara más suave
    ...
    springK = 1.5, // Ajuste para resorte más fuerte
    dampingC = 0.08, // Ajuste para mayor amortiguación
    // Slope Ray setups
  ...
  const setCameraBased = useAnimator((state) => state.setCameraBased);
  const getCameraBased = useAnimator((state) => state.getCameraBased);
  ...
  const idleAnimation = !animated2 ? null : useAnimator((state) => state.idle);
  const walkAnimation = !animated2 ? null : useAnimator((state) => state.walk);
  const runAnimation = !animated2 ? null : useAnimator((state) => state.run);
  const jumpAnimation = !animated2 ? null : useAnimator((state) => state.jump);
  const jumpIdleAnimation = !animated2
    ? null
    : useAnimator((state) => state.jumpIdle);
  const fallAnimation = !animated2 ? null : useAnimator((state) => state.fall);
  const action1Animation = !animated2
    ? null
    : useAnimator((state) => state.action1);
  const action2Animation = !animated2
    ? null
    : useAnimator((state) => state.action2);
  const action3Animation = !animated2
    ? null
    : useAnimator((state) => state.action3);
  const action4Animation = !animated2
    ? null
    : useAnimator((state) => state.action4);
  /**
   * Debug settings
   */
  ...
  const getMoveToPoint = useAnimator((state) => state.getMoveToPoint);
  ...
  const moveCharacter = (_, run, slopeAngle2, movingObjectVelocity2) => {
    if (
      ...
      Math.abs(slopeAngle2) > 0.2 &&
      Math.abs(slopeAngle2) < slopeMaxAngle
    ) {
    ...
    const angleBetweenCharacterDirAndObjectDir =
      movingObjectVelocity2.angleTo(movingDirection);
    /**
    ...
    if (!characterRotated) {
      moveImpulse.set(
        ...
        slopeAngle2 === null || slopeAngle2 == 0
          ? 0
          : movingDirection.y *
              turnVelMultiplier *
              (movingDirection.y > 0
  ...
  useFrame((state, delta) => {
    ...
    if (controllerIndex !== null) {
      ...
      modelEuler.y = ((movingDirection2) =>
        movingDirection2 === null ? modelEuler.y : movingDirection2)(
        getMovingDirection(
          gamepadKeys.forward,
    ...
    if ((jump || button1Pressed) && canJump) {
      ...
      (_a = rayHit.collider.parent()) == null
        ? void 0
        : _a.applyImpulseAtPoint(characterMassForce, standingForcePoint, true);
    }
    ...
    rayHit = world.castRay(
      ...
      (collider) =>
        ...
        !collider.parent().userData.excludeEcctrlRay
    );
    /**Test shape ray */
    ...
    if (slopeRayHit) {
      actualSlopeNormal =
        (_b = slopeRayHit.collider.castRayAndGetNormal(
          slopeRayCast,
          slopeRayLength,
          false
        )) == null
      ...
      if (actualSlopeNormal) {
        ...
        actualSlopeAngle =
          actualSlopeNormalVec == null
            ? void 0
            : actualSlopeNormalVec.angleTo(floorNormal);
      }
    }
    ...
    if (rayHit != null) {
      if (canJump && rayHit.collider.parent()) {
        ...
        (_c = rayHit.collider.parent()) == null
          ? void 0
          : _c.applyImpulseAtPoint(
              characterMassForce,
              standingForcePoint,
              true
            );
  ...
  return /* @__PURE__ */ React.createElement(
    ...
    isModePointToMove &&
      /* @__PURE__ */ React.createElement(CylinderCollider, {
        ref: bodySensorRef,
        sensor: true,
    ...
    /* @__PURE__ */ React.createElement(
      "group",
      { ref: characterModelRef, userData: { camExcludeCollision: true } },
      /* @__PURE__ */ React.createElement(
        "mesh",
        {
          position: [
